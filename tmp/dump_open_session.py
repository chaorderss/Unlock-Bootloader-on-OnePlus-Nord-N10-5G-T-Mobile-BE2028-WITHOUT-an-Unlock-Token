#!/usr/bin/env python3
"""Dump AbstractShell::open_session from libmir1server.so to see
how authorize_session result is used."""
import struct

def read_u32(data, off):
    return struct.unpack_from('<I', data, off)[0]

# Use the ORIGINAL (pre-session-name-patch) or patched — doesn't matter for this function
...
# Also check ShellWrapper::open_session at 0x184f70, 116 bytes
for name, start, size in [("AbstractShell::open_session", 0x17ad60, 272),
                           ("ShellWrapper::open_session", 0x184f70, 200)]:
    print(f"\n=== {name} @ 0x{start:x} ({size} bytes) ===")
    for off in range(start, min(start + size, len(data)), 4):
        insn = read_u32(data, off)
        asm = ""
        note = ""
        ...
        if insn == 0xd65f03c0: asm = "RET"
            ...
            t = off + imm26*4
            sym = plt_sym.get(t, "")
            asm = f"BL <{sym[:70]}>" if sym else f"BL 0x{t:x}"
        elif (insn >> 26) == 0x05:
            imm26 = insn & 0x3ffffff
            if imm26 & 0x2000000: imm26 -= 0x4000000
        ...
        elif (insn & 0x7fc00000) in (0xa9000000,0xa9400000,0xa9800000,0xa9c00000,
                                       0xa8c00000,0x29000000,0x29400000):
            opc = (insn >> 30) & 3; l = (insn >> 22) & 1
            imm7 = (insn >> 15) & 0x7f
            if imm7 & 0x40: imm7 -= 0x80
            rt2 = (insn >> 10) & 0x1f; rn = (insn >> 5) & 0x1f; rt = insn & 0x1f
            scale = 2 + (opc >> 1)
            rp = "X" if opc >= 2 else "W"
            rn_s = "SP" if rn==31 else f"X{rn}"
            asm = f"{'LDP' if l else 'STP'} {rp}{rt}, {rp}{rt2}, [{rn_s}, #{imm7 << scale}]"
        else:
            asm = f"??? 0x{insn:08x}"

        marker = ""
        if "STRB" in asm: marker = " <<<< STRB"
        if asm == "RET": marker = " <<<< RET"
        if "LDRB" in asm: marker = " << LDRB (bool?)"

        print(f"  0x{off:05x}: {insn:08x}  {asm}{marker}")

# Also look for authorize_session in symtab/dynsym
print("\n=== Looking for 'authorize' in symbol names ===")
for sec_name in ['.dynsym', '.symtab']:
    sec = sections.get(sec_name)
    if not sec: continue
    strtab_name = '.dynstr' if sec_name == '.dynsym' else '.strtab'
    strtab = sections.get(strtab_name)
    if not strtab: continue
    strtab_data = data[strtab['offset']:strtab['offset']+strtab['size']]
    ent = sec['entsize'] or 24
    for i in range(sec['size'] // ent):
        soff = sec['offset'] + i * ent
        st_name = struct.unpack_from('<I', data, soff)[0]
        st_value = struct.unpack_from('<Q', data, soff + 8)[0]
        st_size = struct.unpack_from('<Q', data, soff + 16)[0]
        end = strtab_data.find(b'\0', st_name)
        if end > st_name:
            nm = strtab_data[st_name:end].decode('ascii', errors='replace')
            if 'authorize' in nm.lower() or 'add_session' in nm.lower():
                print(f"  [{sec_name}] 0x{st_value:x} size={st_size} {nm}")
